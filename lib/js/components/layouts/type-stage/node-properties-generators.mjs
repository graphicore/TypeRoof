...
import { calculateHorizontalRunion } from "./horizontal-layout-runion.mjs";
...
const horizontalLayoutRunionSyntheticValue = new SyntheticValue(
    calculateHorizontalRunion,
    [ // ...args
        `${LAYOUT}availableWidth`, // availableWidthEn

    ]
);

// TODO: duplicated from properties-generators.mjs
function enToPtNumeric(value, fontSize) {
    return (value / 2) * fontSize;
}
function enToPt(value, fontSize) {
    return `${enToPtNumeric(value, fontSize)}pt`;
}

export function* horizontalLayoutRunionGen(/*inputCascade , hostContext ,*/) {
    yield [`${LAYOUT}_runion`, horizontalLayoutRunionSyntheticValue];

    yield [`${GENERIC}columnCount`, new SyntheticValue(pullFrom('columnCount'), [`${LAYOUT}_runion`], {
            optional: true,
        }),
    ];
    ...
    {
        const pullFromKeys = ['lineLength', 'columnCount', 'columnGutter', 'marginStart', 'marginEnd']
          , rawPropertyTargets = pullFromKeys.map(key=>`${GENERIC}${key}`)
          , ptPropertyTargets = pullFromKeys.map(key=>{
                ...
                return `${GENERIC}${key}/pt`;
            });
        propertyRouting.push(...zip(pullFromKeys, rawPropertyTargets, ptPropertyTargets));
    }
    console.log("propertyRouting", propertyRouting);
    for(const [pullFromKey, rawPropertyTarget, ptPropertyTarget] of propertyRouting) {
        console.log(pullFromKey, rawPropertyTarget, ptPropertyTarget);
        yield [rawPropertyTarget, new SyntheticValue(pullFrom(pullFromKey), [`${LAYOUT}_runion`], {
                optional: true,
            }),
        ];
        if(ptPropertyTarget !== null)
            yield [ptPropertyTarget, new SyntheticValue(enToPt, [
                rawPropertyTarget, `${GENERIC}fontSize`,
            ])];
    }

    yield [`${LAYOUT}columnWidth`,  new SyntheticValue(enToPtNumeric, [`${GENERIC}lineLength`, `${GENERIC}fontSize`])];

    yield [`${LAYOUT}width`, new SyntheticValue(
        (runionData, fontSize)=>enToPtNumeric(pullFrom('fullWidth')(runionData), fontSize),
...
export function* horizontalLayoutNodePropertiesGen(
    ...
    if (algorithm === "HorizontalLayoutRunionModel")
        yield * horizontalLayoutRunionGen(inputCascade, hostContext);
    else
        // "ManualHorizontalLayoutModel" OR undefined (NULL default):
        // empty inputs degrade to fill — columnWidth = availableWidth.
