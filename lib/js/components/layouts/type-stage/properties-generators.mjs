...
function* inlineLengthUnitsGen() {
    for (const enKey of ["lineLength", "columnGutter", "inlineMargins/start", "inlineMargins/end"]) {
        const targetPath = `${GENERIC}${enKey}`;
        yield [`${targetPath}`, new SyntheticValue(createInlineLength, [
            `${targetPath}/value`, `${targetPath}/unit`,
            `${GENERIC}baseFontSize`, `${GENERIC}fontSize`,
        ], { optional: true })];
        ...
        yield [`${targetPath}/pt`, new SyntheticValue(enToPt, [
            targetPath, `${GENERIC}fontSize`,
        ], { optional: true })];
    }
}
...
export function* horizontalLayout(outerTypespecnionAPI, hostInstance) {
    ...
    console.log("horizontalLayout.keys()", ...horizontalLayout.keys());
    // -> instance horizontalLayoutAlgorithmTypeKey horizontalLayoutAlgorithmModel availableHorizontalLayoutAlgorithmTypes
    ...
    // I tried to
    //    `yield [`${HORIZONTAL_LAYOUT}algorithm`, "ManualHorizontalLayoutModel"]`
    // when `algorithm === ForeignKey.NULL` with the assumption:
    // has a good default behavior after a parent was HorizontalLayoutRunionModel
    // HOWEVER, it does not work that way!
    // The question is now how to continue in a child after a column-layout
    // has been applied and there's no new horizontal layout handling defined.
    // So, technically, the answer can only be in the layout properties
    // as the type-spec properties should allow to inherit a column-layout
    // to their type-spec children!.

    if (algorithm === "ManualHorizontalLayoutModel") {
        const manualInstance = horizontalLayout.get("instance").wrapped;
        console.log("manualInstance: " + manualInstance, manualInstance);
        ...
        console.log("runionInstance: " + runionInstance, runionInstance);
