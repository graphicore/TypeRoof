...
import {
    ...
    deserializeManualBlockMarginsModel,
    deserializeManualInlineMarginsModel,
    deserializeInlineLengthValueModel,
} from "../../type-spec/models.mjs";

import { deserializeHorizontalLayoutAlgorithmModel } from "../../type-spec/horizontal-layout-models.mjs";
...
// this will raise.
export function getFallback(fullKey, modelDefaultValue = _NOTDEF) {
...
export function typeSpecGetDefaults(
    ...
    // in comparable functions in motion-stage, however,
    ...
    // These requests come via UIManualAxisLocations:
        ...
        console.log(
            `typeSpecGetDefaults returning getFallback fullKey:${fullKey} modelDefaultValue:`,
            modelDefaultValue,
        );
        return getFallback(fullKey, modelDefaultValue);
    }
...
export function getTypeSpecDefaultsMap(typeSpecDependencies) {
    const defaultTypeSpec = (() => {
            ...
            for (const [fieldName, ppsRecord] of TYPESPEC_PPS_MAP) {
                ...
                if (ppsRecord.prefix == COLOR) {
                    ...
                    ppsRecord.fullKey === `${GENERIC}blockMargins` ||
                    ppsRecord.fullKey === `${GENERIC}inlineMargins`
                ) {
                    const defaultValue = typeSpecGetDefaults(
                            () => null,
                            ppsRecord,
                    ...
                    ppsRecord.fullKey === `${GENERIC}columnGutter` ||
                    ppsRecord.fullKey === `${GENERIC}lineLength`
                ) {
                    const defaultValue = typeSpecGetDefaults(
                            () => null,
                            ppsRecord,
                        ...
                        value = deserializeInlineLengthValueModel(
                            draft.dependencies,
                            defaultValue,
                        );
                    draft.set(fieldName, value);
                } else if (ppsRecord.fullKey === `${GENERIC}horizontalLayout`) {
                    console.log("got " + ppsRecord);
                    const defaultValue = typeSpecGetDefaults(
                        () => null,
                        ppsRecord,
                        fieldName,
                    );
                    const value = deserializeHorizontalLayoutAlgorithmModel(
                        draft.dependencies,
                        defaultValue,
                    );
        ...
        // Demarcation routing matters here as well: inheritance controls
        // (tombstones, re-routes) are meaningless in the defaults context
        // — defaults have no children — and must not land in the local
        // map as literal values (a TOMBSTONE Symbol passed as a value
        // into a SyntheticValue breaks resolution).
        [rawDefaults /*, ignoredDefaultsInheritanceControls */] =
            collectPropertyGeneratorEntries(properties, DEMARCATION_PROPERTY),
