...
export const ColumnCountModel = _AbstractNumberModel.createClass(
    ...
    PositiveIntegerModel = _AbstractNumberModel.createClass("PositiveIntegerModel", {
        defaultValue: 0,
        min: 0,
    ...
    PositiveIntegerOrEmptyModel = _AbstractSimpleOrEmptyModel.createClass(PositiveIntegerModel),
    GrowColumnsModel = _AbstractStructModel.createClass(
        ...
        CoherenceFunction.create(
            ...
            function initGrowColumns({ mode, numericValue }) {
                console.log('mode.isEmpty', mode.isEmpty, mode.isEmpty || mode.value)
                if (mode.isEmpty || mode.value !== "number")
                    numericValue.clear();
                else {
                    // mode.value === 'number'
                    if (numericValue.isEmpty) {
                        console.log("numericValue.constructor.Model.defaultValue", numericValue.constructor.Model.defaultValue);
                        numericValue.value =
