...
// FIXME: instead of importing the models to copy their defaults/values
// in here, the models should be defined using original defaults/values
// from here. It would be a lot simpler for the model dependency graph.
import {
    TextAlignmentModel
...
export const REGISTERED_PROPERTIES = {
    [NUMERIC]: _frozenMapFromObject({
      ...
      , rotation: {default: 0, inherit: false}
    })
  , [THREE_D]: _frozenMapFromObject({
        perspective: {default: 0, min: 0, inherit: false}
      , rotationX: {default: 0, inherit: false}
      , rotationY: {default: 0, inherit: false}
      , rotationZ: {default: 0, inherit: false}
    })
  ...
  , [LANGUAGE]: _frozenMapFromObject({
        languageTag: {
            inherit: true
          , label: 'Language Tag'
            // Can be empty, which means as much as don\'t specify a tag.
          , default: {}
        },
        // This is supposed to be a complete string for the "lang" attribute.
        lang: {
            // it is inherited via the node structure, so we should not
            // have to set it for each element explicitly.
            // I imagine, maybe we can generate it from the {LANGUAGE}language
            // property when a local subtag is set, i.e. an SyntheticValue
            // that is only present when there's a local change and that
            // is then not inherited, but only locally consumed. This is
            // experimental, not necessarily a good idea.
            inherit: false
            // label: "?"// not required as this has no direct UI
          , default: null
        }
    })
    // I'm using complex values here first, yet minLeading and maxLeading
    // are registered in GENERIC however a, b, don't seem to be registered
    // lineWidth and leading within are registered again in GENERIC
    // a and b are of the type LineWidthLeadingModel
  ...
  , [DIMENSION]: _frozenMapFromObject({
        gridAxisSelection: {
            inherit: false // irrelevant so far
          , label: 'Select an Axis'
        }
      , gridAxisType:{
            inherit: false // irrelevant so far
          , label: 'Axis Type'
      ...
      , amountValue: {
          ...
          , label: 'Amount'
          , step: 1
          , ...AmountNumberModel.ppsDefaultSettings
        }
    })
  , [GENERIC]: _frozenMapFromObject({
      ...
      , charGroups: {
            // this is super specific for the contextual interface
            inherit: true,
            label: 'Glyphs'
        }
      , showCellBoxes: {
      ...
      , documentRendererMode:{
            inherit: false // so far only top-level
          , label: 'Renderer'
        }
      , showParameters: {
            inherit: true
      ...
      , lineLength: {
            inherit: true
          , label: 'Line Length' // was Column Width
          // , default: {value: 40, unit: 'baseEn'}
          , min: 0
          , step: 0.1
        ...
        , columnGutter: {
            inherit: true
          , label: 'Column Gutter' // was Column Width
          , default: {value: 2, unit: 'en'}
        }
      , leading: {
            inherit: true
          , label: 'Leading Value'
      ...
      , gridContent: {
       ...
       , blockMargins: {
            // if we inherit this, it should only be applied in leave
            // nodes...
            inherit: true
             // FIXME depends if block direction is vertical, but good enough for now.
          , label: 'Vertical Margins'
            // The gap between adjacent nodes is modeled as margin-block-end
            // on the preceding element, sized by the following element's
            // leading. The renderer resolves 'lineHeightAfter' by reading
            // fontSize and line-height-em from the next sibling's typeSpecnion.
          , default: deepFreeze({end: {value: 1, unit: 'lineHeightAfter'}})
        }
      , inlineMargins: {
            inherit: true
          , label: 'Horizontal Margins'
          , default: deepFreeze({
                start: {value: 0, unit: 'en'}
              , end: {value: 0, unit: 'en'}
            })
        }
      , columnCount: {
      ...
      , horizontalLayout: {
          ...
          , default: deepFreeze({
                horizontalLayoutAlgorithmTypeKey: ForeignKey.NULL
              //   leadingAlgorithmTypeKey:"HorizontalLayoutRunionModel"

              //  horizontalLayoutAlgorithmTypeKey:"ManualHorizontalLayoutModel"
              //, instance: {
              //      inlineMargins: deepFreeze({
              //        start: {value: 0, unit: 'en'}
              //      , end: {value: 0, unit: 'en'}
              //      })
              //    , columnCount: 1
              //    , columnGutter: {value: 0, unit: 'en'}
              //    // We leave this undefined, as the "fill all available
              //    // space" case can't be selected if we inherit a value
              //    // here
              //    // , lineLength: {value: 53, unit: 'baseEn'}
              //  }
            })
        }
    })
